...
- (id)init
{
    ...
    if (self) {
        ...
        self.theme=(int)[[NSUserDefaults standardUserDefaults] integerForKey:@"Theme"] ;
...
-(void)login:(AVUserResultBlock)callback{
    ...
    [AVOSCloudSNS loginWithCallback:^(NSDictionary* object, NSError *error) {
        if (error==nil && object) {
            [VZUser loginWithAuthData:object platform:AVOSCloudSNSPlatformWeiBo block:^(AVUser *user, NSError *error) {
                BOOL needSave=NO;
